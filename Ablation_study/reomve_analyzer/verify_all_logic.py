...
def check_intelligent_prediction():
    ...
    reader_src2 = Path("agent_reader.py").read_text()
    
    # 1. Prediction rules (now in Reader since Analyzer is removed)
    has_rules = "prediction" in reader_src2.lower() or "Prediction" in reader_src2
    checks.append(("Prediction rules", has_rules))
    
    # 2. Confidence levels
    has_confidence = "90%" in reader_src2 and "70-89%" in reader_src2
    checks.append(("Confidence levels", has_confidence))
    
    # 3. Port prediction
    has_port = "Flask" in reader_src2 and "8000" in reader_src2
